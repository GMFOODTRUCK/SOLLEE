...
public class LoginAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request,
		...
		owner.setOwnerPwd(request.getParameter("password"));
		...
		if(clist.get(0)!=null) {
			...
			if(clist.get(0).getCustomerPwd().equals(request.getParameter("password"))) {
				/* 비밀번호 검사 검사 */
				// 고객 로그인 처리
			...
			if(olist.get(0).getOwnerPwd().equals(request.getParameter("password"))) {
				/* 비밀번호 검사 검사 */
				// 업주 로그인 처리
